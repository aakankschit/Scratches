...
# Paths to input data.
pKa_SUBMISSIONS_DIR_PATH = '../microstate_analysis/submissions/'
EXPERIMENTAL_DATA_FILE_PATH = './_experimental_values.csv'
...
def barplot_with_CI_errorbars_colored_by_label(df, x_label, y_label, y_lower_label, y_upper_label, color_label, figsize=False):
    ...
    color3 = "#85C0F9"
    color4 = "#D81B60"
    current_palette = [color0, color1, color2, color3,color4]
    error_color = 'gray'

    # Bar colors
    if color_label == "category":
        #category_list = ["Physical", "Empirical", "Mixed", "Other"]
        category_list = ["QM", "QM+LEC", "QSPR/ML","Physical (QM+LEC)","Other"]#["Physical", "Empirical"]
...
def generate_statistics_tables(submissions, stats_funcs, directory_path, file_base_name,
    ...
    with open(os.path.join(latex_directory_path, file_base_name + '.tex'), 'w') as f:
        ...
        f.write('\end{center}\n'
                ...
                #'- Some pKa predictions were submitted after the submission deadline to be used as a reference method.\n\n'
                '\end{document}\n')
...
if __name__ == '__main__':
    ...
    # Fix charge and unit errors, then convert relative free energies to macro pKa's
    ranked_submissions_pKa, one_to_0transitions = submission_fix_and_convert(ranked_submissions_RFE)

    # Perform the analysis
    output_directory_path='./analysis_outputs_ranked_submissions'
    pKa_submission_collection_file_path = '{}/pKa_submission_collection.csv'.format(output_directory_path)

    collection_pKa = pKaSubmissionCollection(ranked_submissions_pKa,
                                               experimental_data,
